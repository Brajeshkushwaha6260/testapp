...
@api_view(['GET', 'POST', 'DELETE'])
def Ans_Patient(request):
 if request.method=='POST':
    emailid=request.data['emailid']
    prescriptionlist=Answers.objects.all().filter(patient_id=emailid)
    prescription_serializer=AnswerGetSerializer(prescriptionlist,many=True)
